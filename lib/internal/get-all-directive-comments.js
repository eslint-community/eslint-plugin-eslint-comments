...
// Cache of parsed directive comments.
// A single AST is linted by several rules that call `getAllDirectiveComments`
// with different (or no) `additionalDirectives`, and each set yields a
// different parse result, so the cache is keyed on the AST *and* on a
// normalized form of `additionalDirectives`.
const pool = new WeakMap()

/**
 * Build a stable cache key for an `additionalDirectives` list. Order and
 * duplicates do not affect the parse result, so they are normalized away
 * to let equivalent lists share a cache entry.
 *
 * @param {string[]} [additionalDirectives] - Additionally allowed directives.
 * @returns {string} The cache key (`""` when there are no additional directives).
 */
function toAdditionalDirectivesCacheKey(additionalDirectives) {
    return additionalDirectives && additionalDirectives.length
        ? JSON.stringify([...new Set(additionalDirectives)].sort())
        : ""
}
...
module.exports = {
    ...
    getAllDirectiveComments(context, additionalDirectives) {
        ...
        let astCache = pool.get(sourceCode.ast)
        if (astCache == null) {
            astCache = new Map()
            pool.set(sourceCode.ast, astCache)
        }
        const cacheKey = toAdditionalDirectivesCacheKey(additionalDirectives)
        let result = astCache.get(cacheKey)
